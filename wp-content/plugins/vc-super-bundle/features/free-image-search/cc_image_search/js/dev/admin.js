...
CCImageSearch = {
	...
	providers: {
		...
		pixabay: {
			api: 'https://pixabay.com/api/?key=281886-58afb50cd9c4019517ce11b78&search_term={keyword}&image_type=photo&response_group=high_resolution&per_page={per_page}&page={page}',
			formURL: function( args ) {
		...
		fivehundredpx: {
			api: 'https://api.500px.com/v1/photos/search?term={keyword}&page={page}&rpp={per_page}&image_size=1080,1600,2048&license_type={license}&consumer_key=2nPLJW2MHZDBwghYlITgJpzzdmEZNOuG8VmunwbK',
			formURL: function( args ) {
				...
				url = url.replace( /\{page\}/, args.page );
		  		...
		  		// Flickr licenses: https://www.flickr.com/services/api/flickr.photos.licenses.getInfo.html
		  		if ( args.license === 'noncommercial' ) {
		  			url = url.replace( /\{license\}/, '1,2,3,4,5,6,7,8' );
		  		} else if ( args.license === 'noattribution' ) {
		  			url = url.replace( /\{license\}/, '7,8' );
		  		} else {
		  			url = url.replace( /\{license\}/, '4,5,6,7,8' );
		  		}
		    ...
		    onload: function( response, resultCallback ) {
				...
				for ( var i = 0; i < data.photos.length; i++ ) {
					...
					var image = {
						provider: 'fivehundredpx',
						provider_name: '500px',
						provider_link: 'http://500px.com',
						date: hit.created_at,
						title: hit.name,
						user: hit.user.fullname,
						user_link: 'https://500px.com/' + hit.username,
						url: 'https://500px.com/' + hit.url,
						// 	  license: 'Creative Commons CC0 / Public Domain',
						// 	  license_link: 'https://creativecommons.org/publicdomain/zero/1.0/',
						// 	  license_shortname: 'CC BY-NC-SA',
						badges: [],
					...
					// 500px licenses: https://github.com/500px/api-documentation/blob/master/basics/formats_and_terms.md#license-types
					if ( hit.license_type === 1 ) {
						image.license = 'Creative Commons Attribution-NonCommercial';
						image.license_link = 'http://creativecommons.org/licenses/by-nc/2.0/';
						image.license_shortname = 'CC BY-NC';
						image.badges.push('attribution');
						image.badges.push('noncommercial');
					} else if ( hit.license_type === 2 ) {
						image.license = 'Creative Commons Attribution-NonCommercial-NoDerivs';
						image.license_link = 'http://creativecommons.org/licenses/by-nc-nd/2.0/';
						image.license_shortname = 'CC BY-NC-ND';
						image.badges.push('attribution');
						image.badges.push('noncommercial');
					} else if ( hit.license_type === 3 ) {
						image.license = 'Creative Commons Attribution-NonCommercial-ShareAlike';
						image.license_link = 'http://creativecommons.org/licenses/by-nc-sa/2.0/';
						image.license_shortname = 'CC BY-NC-SA';
						image.badges.push('attribution');
						image.badges.push('noncommercial');
					} else if ( hit.license_type === 4 ) {
						image.license = 'Creative Commons Attribution';
						image.license_link = 'http://creativecommons.org/licenses/by/2.0/';
						image.license_shortname = 'CC BY';
						image.badges.push('attribution');
					} else if ( hit.license_type === 5 ) {
						image.license = 'Creative Commons Attribution-NoDerivs';
						image.license_link = 'http://creativecommons.org/licenses/by-nd/2.0/';
						image.license_shortname = 'CC BY-ND';
						image.badges.push('attribution');
					} else if ( hit.license_type === 6 ) {
						image.license = 'Creative Commons Attribution-ShareAlike';
						image.license_link = 'http://creativecommons.org/licenses/by-sa/2.0/';
						image.license_shortname = 'CC BY-SA';
						image.badges.push('attribution');
					} else if ( hit.license_type === 7 ) {
						image.license = 'Creative Commons License Public Domain Mark 1.0';
						image.license_link = 'https://creativecommons.org/publicdomain/mark/1.0/';
						image.license_shortname = 'CC0';
						image.badges.push('zero');
						image.badges.push('warning');
					} else if ( hit.license_type === 8 ) {
						image.license = 'Creative Commons License Public Domain Dedication';
						image.license_link = 'http://flickr.com/commons/usage/';
						image.license_shortname = 'CC0';
						image.badges.push('zero');
					}

					// Width & height are not from the images that we got.
					var w1, w2, h1, h2;
					w1 = parseInt( hit.width, 10 );
					h1 = parseInt( hit.height, 10 );

					// Get all the images.
					for ( var k = 0; k < hit.images.length; k++ ) {

						// The size that we get here is the longest width or height.
						var size = parseInt( hit.images[ k ].size, 10 );

						// Compute for the actual width & height of the image.
						if ( w1 > h1 ) {
							// Then size is the width.
							w2 = size;
							h2 = parseInt( h1 / w1 * w2, 10 );
						} else {
							// Then size is the height.
							h2 = size;
							w2 = parseInt( w1 / h1 * h2, 10 );
						}

						image.sizes.push({
							url: hit.images[ k ].url,
							width: w2,
							height: h2
						});
					}
		...
		pexels: {
		    api: 'http://api.pexels.com/v1/search?query={keyword}&page={page}&per_page={per_page}',
		    formURL: function( args ) {
