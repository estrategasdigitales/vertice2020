...
';
		...
		$product_visibility_term_ids = array();
		...
		$paged = absint( empty( $_GET['product-page'] ) ? $paged : $_GET['product-page'] ); // WPCS: input var ok, CSRF ok.
		...
		if('masonry'!==$display){
			if( $query_type === '2' &&  (is_post_type_archive( 'product' ) || is_tax( get_object_taxonomies( 'product' ) ) ) ){
				$r = $wp_query;
			}else{
		...
		$output .= '<div class="dhvc-woo-row-fluid dhvc-woo-' . $display . '-list"' . ($display == 'masonry' ? ' data-masonry-gutter="' . absint ( $masonry_gutter ) . '"' : '') . '' . ($display == 'carousel' ? ' data-carousel_autoplay="'.absint($carousel_autoplay).'" data-carousel_autoplay_timeout="'.absint($carousel_autoplay_timeout).'" data-items="' . absint ( $post_per_row ) . '"' : '').($display == 'carousel' && ! empty ( $show_carousel_pagination ) ? ' data-pagination="true"' : '') . '>';
		...
		if('grid' === $display && $r->have_posts() && !empty($use_default_style_of_theme) ){
			...
			if(function_exists('wc_setup_loop')){
				...
				wc_setup_loop( array(
					...
					'total_pages'  => (int) $r->max_num_pages,
					'per_page'     => (int) $r->get( 'posts_per_page' ),
					'current_page' => (int) max( 1, $r->get( 'paged', 1 ) ),
				) );
			}
			ob_start();
		...
		while ( $r->have_posts () ) :
			...
			if (has_post_thumbnail ()) {
				...
				if(!empty($thumbnail_height) && !empty($thumbnail_width)){
					...
					$thumbnail_data = dhvc_woo_getImageBySize(array( 'post_id' => $product->get_id(), 'thumb_size' => $thumb_size ));
				...
				$output_image .=  apply_filters ( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image"><img src="%s" alt="Placeholder" /></a>', esc_url ( get_permalink () ), wc_placeholder_img_src () ), $product->get_id() );
			}
			$output_image .= '</div>';
			endif;
			...
			if($display === 'list'){
				...
				if($add_cart_price_style === '1column'){
					...
					$output_price = '';
					if ($hide_price !== '0') :
						$output_price .= '<div class="dhvc-woo-price dhvc-woo-span12 derecha">';
						$output_price .= $product->get_price_html ();
						$output_price .= '</div>';
					endif;
					$output .= apply_filters('dhvc_woo_price', $output_price, $product, $display);
					$output_addtocart = '';
					if ($hide_addtocart !=='0') :
						$output_addtocart .= '<div class="dhvc-woo-addtocart dhvc-woo-span12 derecha">';
						ob_start();
						//wc_get_template( 'loop/add-to-cart.php' );
						woocommerce_template_loop_add_to_cart();
						$add_to_cart = ob_get_clean();
	...
	public function template_loop_product_thumbnail($product,$size='shop_catalog') {
		...
		if($size == 'shop_catalog'){
			$thumbnail_data = get_the_post_thumbnail( $product->get_id(), 'shop_catalog' );
			return '<div class="dhvc-woo-thumbnail'.($frist != '' ? ' dhvc-woo-front-thumbnail':'').'">' . $thumbnail_data .'</div>';
		}else{
			$thumbnail_data = dhvc_woo_getImageBySize(array( 'post_id' => $product->get_id(), 'thumb_size' => $size ));
