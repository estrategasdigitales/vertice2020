...
class DHVC_Woo_Page_Vc_Frontend_Editor extends Vc_Frontend_Editor{
	...
	public function vc_frontend_editor_iframe_url($url){
		return $url.'&dhvc_woo_product_page_editable=true&dhwc_tempalte_id='.get_the_ID();
	}
	...
	public function __need_set_post_by_tempalte($template_id){
		global $post;
		$this->post_id = $template_id;
	...
	public function buildEditablePage() {
		...
		if(property_exists('Vc_Base', 'shared_templates'))
	...
	function loadShortcodes() {
		if ( vc_is_page_editable() && self::inlineEnabled() ) {
			...
			if ( 'vc_load_shortcode' === $action ) {
				...
				! defined( 'CONCATENATE_SCRIPTS' ) && define( 'CONCATENATE_SCRIPTS', false );
				ob_start();
				if(vc_get_param('dhwc_tempalte_id')){
					$this->__need_set_post_by_tempalte(vc_get_param('dhwc_tempalte_id'));
				}else{
					$this->setPost();
				}
