    ...
    ( function ($) {
        ...
        var reasonsHtml = <?php echo json_encode( $reasons_list_items_html ); ?>,
            modalHtml =
                ...
                + '         <div class="ts-modal-panel active" data-panel-id="reasons"><h3><strong><?php printf( Wcap_TS_deactivate::load_str( 'deactivation-share-reason' ) ); ?>:</strong></h3><ul id="reasons-list">' + reasonsHtml + '</ul></div>'
                + '     </div>'
                + '     <div class="ts-modal-footer">'
                + '         <a href="#" class="button button-secondary button-deactivate"></a>'
                + '         <a href="#" class="button button-primary button-close"><?php printf( Wcap_TS_deactivate::load_str( 'deactivation-modal-button-cancel' ) ); ?></a>'
                + '     </div>'
                + ' </div>'
                + '</div>',
            $modal = $(modalHtml),
        ...
        function registerEventHandlers() {
            ...
            $modal.on('click', 'input[type="radio"]', function () {
                ...
                $modal.find('.button-deactivate').text('<?php printf( Wcap_TS_deactivate::load_str( 'deactivation-modal-button-submit' ) ); ?>');
        ...
        function updateButtonLabels() {
            ...
            if ('confirm' === getCurrentPanel()) {
                $deactivateButton.text('<?php printf( Wcap_TS_deactivate::load_str( 'deactivation-modal-button-confirm' ) ); ?>');
            } else {
                var $radio = $('input[type="radio"]:checked');
                if( $radio.length == 0 ) {
                    $deactivateButton.text('<?php printf( Wcap_TS_deactivate::load_str( 'deactivation-modal-button-deactivate' ) ); ?>');
                } else {
