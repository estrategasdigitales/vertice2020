...
class Iconic_WAS_Attributes {
	...
	public function modify_attribute_html( $html, $args ) {
		...
		foreach ( $args['options'] as $label => $options ) {
			...
			$oos_terms         = $has_any_variation ? array() : $iconic_was->products_class()->get_out_of_stock_terms( $_product->get_id(), $args['attribute'] );
	...
	public function modify_layered_nav_term_html( $term_html, $term, $link, $count ) {
		...
		if ( $link ) {
			$term_html = sprintf( '<a href="%s" class="%s">%s</a>', esc_url( $link ), esc_attr( implode( ' ', $class ) ), $swatch );
		} else {
